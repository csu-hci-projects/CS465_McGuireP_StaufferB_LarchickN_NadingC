...
\section{Methodology}
Our research compares two interfaces for managing task lists: an augmented reality prototype viewed via head-mounted displays (e.g., Meta Quest 2) and a traditional smartphone-based list application. Participants will complete a series of predefined tasks in different real-world contexts such as grocery shopping, fishing, or skiing. Each participant will perform the tasks once using the AR interface and once using the smartphone list, with the order counterbalanced to avoid learning effects. 
...
This study seeks to compare, contrast, and analyze different extended reality approaches to user interface design when applied to creating a task list. Due to the broad area of this topic and the multiple applicable approaches, we created multiple prototypes. This enables us to then compare the prototypes against one another in studies to find what version of the task list is preferred by users. 
