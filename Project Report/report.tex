...
To evaluate the prototype, we conducted a user study where participants performed a simulated grocery shopping task within a virtual reality environment. Prior to beginning, each participant received a brief tutorial on the interaction techniques and the virtual space they would be using. The goal was to minimize any bias caused by varying levels of VR familiarity.

The task involved navigating a virtual grocery store and collecting items listed on a task list. Participants were instructed to search store aisles, locate each item, and check it off the list using their preferred input method. There were five different types of "groceries" varying by color and shape. The user could pull up the list user interface on either wrist through controller input. Once looking at the list UI, participants were free to switch between the three supported interaction modalities: gaze, gesture, and controller-based interaction. The goal is to collect all items listed and cross them off, using whatever input is most natural, as they are collected. 

Throughout the session, system logs captured all user interactions, including task completion time, selection events, and errors such as accidental selections. After completing the task, participants filled out a post-task survey rating the ease of use, physical and cognitive effort, and overall satisfaction with each input method. Open-ended questions also allowed them to reflect on preferences and share qualitative feedback.

Each session lasted approximately 20 minutes, including the tutorial, task performance, and follow-up survey
